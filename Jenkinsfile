 ...
 */
def GIT_REPO_URL = 'https://github.com/gizafoundation/zowe-cli-cics-plugin.git'
 ...
 * the builds
 */
def PRODUCT_NAME = "brightside-cics-plugin"
...
pipeline {
    ...
    stages {
         ...
         * OUTPUTS
         * -------
         * able to issue zowe command.
         ************************************************************************/
        stage('Install Zowe CLI @next version') {
            when {
                allOf {
                    expression {
        ...
        stage('Test: System') {
            ...
            environment {
                ...
                SYSTEM_TEST_PROPERTIES = "***REMOVED***"
            }
            steps {
                timeout(time: 30, unit: 'MINUTES') {
                        echo 'System Test'
                ...
                timeout(time: 5, unit: 'MINUTES') {
                    ...
                    withCredentials([usernamePassword(credentialsId: ARTIFACTORY_CREDENTIALS_ID, usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                        script {
                            ...
                            // Install the plugin
                            if (BRANCH_NAME == MASTER_BRANCH) {
                                sh "bright plugins install ${TARGET_SCOPE}/cics@next"
                            } else {
                                sh "bright plugins install ${TARGET_SCOPE}/cics@latest"
                            }
